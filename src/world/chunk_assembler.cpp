...
void ChunkAssembler::shift_chunk(MapChunk *chunk,
  ...
  for (size_t i{0}; i < chunk->width(); i++) {
    for (size_t j{0}; j < chunk->height; j++) {
      ...
      if (shifted_index >= 0 && shifted_index < chunk->height) {
        chunk->at(i).at(j) = chunk->at(i).at(shifted_index);
      } else if (shifted_index < 0) {
        chunk->at(i).at(j).value() = MapUnit::nothing;
      } else if (shifted_index > chunk->height) {
        chunk->at(i).at(j).value() = chunk->at(i).at(chunk->height - 1).value();
      }
    }
  }
}
